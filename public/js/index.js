    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            if (userRole !== 'owner') {
                ...
                console.log(`User ${user.uid} is an owner. Skipping customer upsert in index.js.`);
    ...
    async function upsertUser(user, role) {
        try {
            ...
            if (!response.ok) {
                ...
                console.log(`User ${user.uid} profile upserted successfully as '${role}'.`);
            }
        } catch (error) {
        ...
        myRestaurantButton.onclick = async () => {
            ...
            if (user) {
                ...
                if (userRole === 'owner') {
                    ...
                    showToast("You do not have permission to access the restaurant dashboard.", "warning");
                    myRestaurantButton.textContent = "Mi Restaurante";
                    myRestaurantButton.disabled = false;
                }
            } else {
    ...
    async function loadRestaurants(reset = false) {
        ...
        try {
            ...
            if (restaurants.length === 0 && reset) {
             ...
             <h4>${restaurant.name.length > 25 ? restaurant.name.substring(0, 25) + '...' : restaurant.name}</h4>
